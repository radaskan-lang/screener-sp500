...
def update_paper_results():
    """
    Met à jour automatiquement les trades paper ouverts
    avec les prix actuels de yfinance.
    """
    trades = load_paper_trades()
    updated = False
    ...
    for trade in trades:
        ...
        try:
            ...
            # Vérifier si stop ou target atteint
            recent_lows  = hist["Low"].values
            recent_highs = hist["High"].values

            hit_stop   = any(l <= stop   for l in recent_lows)
            hit_target = any(h >= target for h in recent_highs)

            if hit_stop and not hit_target:
                trade["exit_price"] = stop
                trade["exit_date"]  = datetime.now().strftime("%Y-%m-%d")
                trade["pnl_pct"]    = round((stop - entry) / entry * 100, 2)
                trade["result"]     = "LOSS"
                trade["status"]     = "CLOSED"
                updated = True
                ...
                # Trade toujours ouvert — P&L actuel
