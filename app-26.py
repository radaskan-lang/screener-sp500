    ...
    if report.empty:
        ...
        for idx, row in report.iterrows():
            ...
            st.markdown(
                ...
                f'<span style="color:{rank_color};font-size:1.1rem;font-weight:bold;">#{rank}</span>'
                f' <span style="color:#4a90d0;font-size:1.1rem;font-weight:bold;">{row.get("Ticker","")}</span>'
                f' <span style="color:#94a3b8;font-size:0.85rem;">{row.get("Sector","")}</span>'
                f' <span style="float:right;color:#00ff88;font-size:0.8rem;">{conv_lbl}</span>'
                f'</div>',
                unsafe_allow_html=True
            )
            ...
            if signals_on:
                st.caption("Signaux: " + signals_on[:120])
...
if "scan_report" in st.session_state and not st.session_state["scan_report"].empty:
    ...
    st.markdown("---")
    st.markdown(f"## Top {len(_report)} Trades — Semaine du {datetime.now().strftime('%d %B %Y')}")
    ...
    for _idx, _row in _report.iterrows():
        ...
        _ci.markdown(
            f"**#{_rank} {_ticker}** | Score: `{_score}` | Conv: `{_conv}/7`  \n"
