...
include_nasdaq=st.checkbox("Inclure Nasdaq 100",value=False,key="inc_ndx",help="Ajoute ~35 titres Nasdaq")
include_russell=st.checkbox("Inclure Russell 200",value=False,key="inc_russ",help="Ajoute ~50 titres mid-cap Russell")

if st.button(f" Lancer  S&P 500 complet ({len(SP500_TICKERS)} actions)"):

    tickers_to_analyze=SP500_TICKERS+(NASDAQ100_EXTRA if include_nasdaq else [])+(RUSSELL200_EXTRA if include_russell else [])
