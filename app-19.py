...
if st.button(f" Lancer  S&P 500 complet ({len(SP500_TICKERS)} actions)"):
    ...
    df["Conv_Bar"]   = conv_results.apply(lambda x: x.get("conv_bar", ""))
    df["Conv_Label"] = conv_results.apply(lambda x: x.get("label", ""))
    df["Conv_Color"] = conv_results.apply(lambda x: x.get("color", "#64748b"))
