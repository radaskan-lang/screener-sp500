...
# Session curl_cffi partagee pour contourner Yahoo Finance 401
try:
...
with st.sidebar:
    ...
    st.markdown("---")
    if GITHUB_STORAGE:
        st.markdown('<span style="color:#00ff88"> GitHub sync actif</span>', unsafe_allow_html=True)
        if st.button("Tester GitHub", key="test_gh"):
            try:
                from github_storage import save_to_github
                ok = save_to_github({"test": "ok"}, "data/test.json")
                if ok:
                    st.success("GitHub: ecriture OK!")
                else:
                    st.error("GitHub: ecriture ECHEC")
            except Exception as e:
                st.error(f"GitHub erreur: {str(e)[:100]}")
    else:
        st.markdown('<span style="color:#f87171"> GitHub sync inactif</span>', unsafe_allow_html=True)
...
if st.button(f" Lancer  S&P 500 complet ({len(SP500_TICKERS)} actions)"):
    ...
    if report.empty:
        ...
        for idx, row in report.iterrows():
            ...
            # Fiche native - zero HTML
            border = "#ffd700" if n_sig>=6 else "#00ff88" if n_sig>=5 else "#1e3a5f"
            st.markdown(
                f'<div style="border:2px solid {border};border-radius:10px;'
                f'padding:14px;margin-bottom:10px;background:#111827;">'
                f'<b style="color:{rank_color}">#{rank}</b>'
                f' <b style="color:#4a90d0;">{row.get("Ticker","")}</b>'
                f' <span style="color:#94a3b8;font-size:0.85rem;">{row.get("Sector","")}</span>'
                f' <span style="float:right;color:#00ff88;">{conv_lbl}</span>'
                f'</div>',
                unsafe_allow_html=True
            )
            c1,c2,c3,c4 = st.columns(4)
            c1.metric("Prix", f"${row.get('Prix',0)}")
            c2.metric("Score", f"{score_fin}/100")
            c3.metric("Conv", f"{n_sig}/7")
            c4.metric("R/R", f"{rr}:1" if rr else "--")
            n1,n2,n3 = st.columns(3)
            n1.metric("Entree", f"${row.get('Entree',0)}")
            n2.metric("Stop", f"${row.get('Stop',0)}")
            n3.metric("Target", f"${row.get('Target',0)}")
            adx_v=row.get("ADX",0); obv_v=str(row.get("OBV_Trend","") or "")
            adx_c="#00ff88" if adx_v>=25 else "#fbbf24" if adx_v>=15 else "#f87171"
            obv_c="#00ff88" if obv_v=="HAUSSIER" else "#f87171" if obv_v=="BAISSIER" else "#94a3b8"
            st.markdown(f'ADX: <span style="color:{adx_c}">{adx_v}</span> | OBV: <span style="color:{obv_c}">{obv_v}</span>', unsafe_allow_html=True)
...
# Cases a cocher depuis le dernier scan
if "scan_report" in st.session_state and not st.session_state["scan_report"].empty:
    _report = st.session_state["scan_report"]
    _regime = st.session_state.get("scan_regime", "HAUSSIER")
    _df_full = st.session_state.get("scan_df", _report)
    st.markdown("---")
    st.markdown("## Top Trades")
    if HAS_SIMULATIONS:
        _sim_options = {v["name"]: k for k,v in SIMULATION_CONFIGS.items()}
        _sim_name = st.selectbox("Simulation", list(_sim_options.keys()), key="sim_sel")
        _sim_key  = _sim_options[_sim_name]
        _sim_cfg  = SIMULATION_CONFIGS[_sim_key]
        st.caption(f"Score:{_sim_cfg['score_min']}+ Conv:{_sim_cfg['conv_min']}/7 R/R:{_sim_cfg['rr_min']} ADX:{_sim_cfg['adx_min']} Fib:{'ON' if _sim_cfg['fibonacci'] else 'OFF'}")
        _report_sim = filter_for_simulation(_df_full, _sim_key)
    else:
        _sim_key = "Sim1"; _report_sim = _report
    st.markdown(f"**Marche {_regime} | {_sim_key} | Selectionner pour Paper Trading :**")
    _selected = []
    for _idx, _row in _report_sim.iterrows():
        _rank=_idx+1; _ticker=str(_row.get("Ticker",""))
        _score=_row.get("Score_Final",0); _entry=_row.get("Entree",0)
        _stop=_row.get("Stop",0); _target=_row.get("Target",0)
        _rr=_row.get("RR_Ratio",0); _sector=str(_row.get("Sector","N/A"))
        _conv=int(_row.get("Conv_N",0)); _adx=_row.get("ADX",0)
        _obv=str(_row.get("OBV_Trend","") or "")
        _cc,_ci = st.columns([0.12,0.88])
        _chk = _cc.checkbox("select", key=f"ptc_{_sim_key}_{_ticker}_{_rank}", label_visibility="hidden")
        _ci.markdown(f"**#{_rank} {_ticker}** | Score:`{_score}` Conv:`{_conv}/7` ADX:`{_adx}` OBV:`{_obv}`  \nEntree:`${_entry}` Stop:`${_stop}` Target:`${_target}` R/R:`{_rr}:1`")
        if _chk:
            _selected.append({"ticker":_ticker,"entry_price":_entry,"stop_price":_stop,"target_price":_target,"conv_n":_conv,"score":_score,"sector":_sector,"simulation":_sim_key,"adx":_adx,"obv_trend":_obv})
    if _selected:
        _strat=st.selectbox("Strategie",["A +5%","B +7%","C Vendredi","D Stop 3%","E Stop 5%","T3 Trailing 3%","T5 Trailing 5%"],index=2,key=f"pt_strat_{_sim_key}")
        if st.button(f"Ajouter {len(_selected)} trade(s) - {_sim_key}",key=f"pt_add_{_sim_key}"):
            for _t in _selected:
                add_paper_trade(ticker=_t["ticker"],entry_price=_t["entry_price"],stop_price=_t["stop_price"],target_price=_t["target_price"],conv_n=_t["conv_n"],score=_t["score"],strategy=_strat,sector=_t["sector"],week_date=datetime.now().strftime("%Y-W%V"))
                # Tag simulation manuellement
                try:
                    from trading_tools import load_paper_trades, save_paper_trades
                    _trades = load_paper_trades()
                    for _tr in _trades:
                        if _tr.get("ticker") == _t["ticker"] and _tr.get("status") == "OPEN":
                            _tr["simulation"] = _t.get("simulation","Sim1")
                    save_paper_trades(_trades)
                except Exception:
                    pass
            st.success(f"{len(_selected)} trade(s) ajoutes - {_sim_key}!"); st.rerun()
...
with pt_tab:
    ...
    if HAS_SIMULATIONS:
        _all_t = update_paper_results()
        if any(t.get("simulation") for t in _all_t):
            st.markdown("#### Comparaison simulations")
            _sims=[get_simulation_summary(_all_t,k) for k in SIMULATION_CONFIGS.keys()]
            _sims=[s for s in _sims if s["n_closed"]>0 or s["n_open"]>0]
            if _sims:
                _sc=st.columns(len(_sims))
                for _i,_s in enumerate(_sims):
                    with _sc[_i]:
                        _clr=SIMULATION_CONFIGS.get(_s["sim_key"],{}).get("color","#4a90d0")
                        st.markdown(f'<b style="color:{_clr}">{_s["name"][:10]}</b>',unsafe_allow_html=True)
                        st.metric("Win Rate",f'{_s["win_rate"]}%')
                        st.metric("P&L moy",f'{_s["avg_pnl"]}%')
                        st.metric("Trades",f'{_s["n_closed"]}F/{_s["n_open"]}O')
                st.markdown("---")
    ...
    with st.expander("Ajouter un trade fictif", expanded=False):
        ...
        with pc:
            pt_strat  = st.selectbox("Strategie", ["A +5%","B +7%","C Vendredi","D Stop 3%","E Stop 5%","T3 Trailing 3%","T5 Trailing 5%"], key="pt_sr")
...
with journal_tab:
    ...
    with st.expander("Enregistrer un trade reel", expanded=True):
        ...
        with jb:
            ...
            j_strat  = st.selectbox("Strategie", ["A +5%","B +7%","C Vendredi","D Stop 3%","E Stop 5%","T3 Trailing 3%","T5 Trailing 5%"], index=2, key="j_sr")
