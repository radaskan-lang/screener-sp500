...
if "scan_df" in st.session_state and not st.session_state["scan_df"].empty:
    ...
    if report.empty:
        ...
        st.markdown("**Selectionner les trades pour le Paper Trading :**")
        selected_trades = []
        check_cols = st.columns([0.5, 1.5, 1, 1, 1, 1, 1])
        check_cols[0].markdown("**#**")
        check_cols[1].markdown("**Ticker**")
        check_cols[2].markdown("**Score**")
        check_cols[3].markdown("**Entree**")
        check_cols[4].markdown("**Stop**")
        check_cols[5].markdown("**Target**")
        check_cols[6].markdown("**R/R**")
        ...
        for idx, row in report.iterrows():
            ...
            checked = cols[0].checkbox("", key=f"pt_check_{ticker}_{rank}")
            cols[1].markdown(f"**{ticker}**")
            cols[2].markdown(f"{score}")
            cols[3].markdown(f"${entry}")
            cols[4].markdown(f"${stop}")
            cols[5].markdown(f"${target}")
            cols[6].markdown(f"{rr}:1")
        ...
        if selected_trades:
            pt_strat_bulk = st.selectbox(
                "Strategie pour les trades selectionnes",
            ...
            if st.button(f"Ajouter {len(selected_trades)} trade(s) au Paper Trading", key="pt_bulk_add"):
                from trading_tools import add_paper_trade
                ...
                st.success(f"{len(selected_trades)} trade(s) ajoutes au Paper Trading!")
                st.rerun()
