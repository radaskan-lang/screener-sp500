...
def ai_score(row):
    ...
    try:
        ...
        ma20_cross = bool(row.get("MA20_Cross", False))
        if ma20 > 0:
            if ma20_cross: score += 10; reasons.append("Recassement MA20")
            elif price > ma20 > ma50: score += 5; reasons.append("Prix > MA20 > MA50")
...
if st.button(f" Lancer  S&P 500 complet ({len(SP500_TICKERS)} actions)"):
    ...
    if use_prefilter:
        ...
        def pf_cb(pct, msg=""):
            try:
                pf_prog.progress(min(1.0, max(0.0, float(pct))))
            except Exception: pass
    ...
    conv_res = df.apply(lambda row: calc_convergence(row), axis=1)
    ...
    if use_claude and api_key:
        ...
        for _, row in top5.iterrows():
            ...
            st.markdown(
                f"**{row.get('Ticker','')}** {sig} "
                f"Score {row.get('Score_Final','')}/100 | "
                f"Conv {row.get('Conv_N','')}/7 | R/R {row.get('RR_Ratio','')}:1"
            )
...
if "scan_report" in st.session_state and not st.session_state["scan_report"].empty:
    ...
    st.markdown("---")
    st.markdown(f"## Top {len(_report)} Trades")
    st.markdown(f"**Marche {_regime}** | Selectionner pour Paper Trading :")
    _selected = []
    for _idx, _row in _report.iterrows():
        _rank=_idx+1; _ticker=str(_row.get("Ticker",""))
        _score=_row.get("Score_Final",_row.get("AI Score Ajuste",0))
        _entry=_row.get("Entree",_row.get("Prix",0)); _stop=_row.get("Stop",0)
        _target=_row.get("Target",0); _rr=_row.get("RR_Ratio",0)
        _sector=str(_row.get("Sector","N/A")); _conv=int(_row.get("Conv_N",0))
        _cc,_ci = st.columns([0.12,0.88])
        _chk = _cc.checkbox("", key=f"ptc_{_ticker}_{_rank}", label_visibility="collapsed")
        _ci.markdown(f"**#{_rank} {_ticker}** | Score: `{_score}` | Conv: `{_conv}/7`  \nEntree: `${_entry}` Stop: `${_stop}` Target: `${_target}` R/R: `{_rr}:1`")
        if _chk:
            _selected.append({"ticker":_ticker,"entry_price":_entry,"stop_price":_stop,"target_price":_target,"conv_n":_conv,"score":_score,"sector":_sector})
    if _selected:
        _strat = st.selectbox("Strategie",["A +5%","B +7%","C Vendredi","D Stop 3%","E Stop 5%"],index=2,key="pt_ss_strat")
        if st.button(f"Ajouter {len(_selected)} trade(s)",key="pt_ss_add"):
            for _t in _selected:
            ...
            st.success(f"{len(_selected)} trade(s) ajoutes!"); st.rerun()
