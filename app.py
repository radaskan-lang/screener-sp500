...
def fetch(ticker):
 ...
 "VOL_Ratio":     vol_anom.get("vol_ratio",1.0) if vol_anom else 1.0,
             "VOL_Score":     vol_anom.get("score",0) if vol_anom else 0,
            # Gap
 ...
 "ID_Mom_Score":   mom_data["score"] if mom_data else 0,
             # Placeholders - calcules dans fetch_parallel
             "Conv_N":        0,
             "Conv_Bar":      "",
             "Conv_Label":    "",
             "Conv_Color":    "#64748b",
...
# Afficher le rapport depuis session_state (persiste entre interactions)
if "scan_df" in st.session_state and not st.session_state["scan_df"].empty:
    if "scan_regime" not in locals():
        regime     = st.session_state.get("scan_regime", "HAUSSIER")
        min_signals= 3
        top_n      = 10
        min_score  = 50
    ...
    if report.empty:
        ...
        # Cases a cocher pour paper trading - format compact mobile
        st.markdown("**Selectionner pour Paper Trading :**")
        selected_trades = []

        for idx, row in report.head(10).iterrows():
            rank   = idx + 1
            ...
            conv_n = int(row.get("Conv_N", 0))

            col_check, col_info = st.columns([0.15, 0.85])
            checked = col_check.checkbox("", key=f"pt_check_{ticker}_{rank}", label_visibility="collapsed")
            col_info.markdown(
                f"**#{rank} {ticker}** &nbsp;|&nbsp; Score: `{score}` &nbsp;|&nbsp; Conv: `{conv_n}/6`  \n"
                f"Entree: `${entry}` &nbsp; Stop: `${stop}` &nbsp; Target: `${target}` &nbsp; R/R: `{rr}:1`"
            )
            ...
            if checked:
                selected_trades.append({
                    ...
                    "conv_n": conv_n, "score": score,
                    "sector": sector, "rr": rr,
                })

        if selected_trades:
            pt_strat_bulk = st.selectbox(
                "Strategie",
                ["A +5%","B +7%","C Vendredi","D Stop 3%","E Stop 5%"],
                index=2, key="pt_bulk_strat"
            )
            if st.button(f"Ajouter {len(selected_trades)} trade(s) au Paper Trading", key="pt_bulk_add"):
                for t in selected_trades:
                    add_paper_trade(
                        ticker=t["ticker"], entry_price=t["entry_price"],
                        stop_price=t["stop_price"], target_price=t["target_price"],
                        conv_n=t["conv_n"], score=t["score"],
                        strategy=pt_strat_bulk, sector=t["sector"],
                        week_date=datetime.now().strftime("%Y-W%V")
                    )
                st.success(f"{len(selected_trades)} trade(s) ajoutes!")
                st.rerun()
 ...
 " Tableau complet",
    ...
    with col_exp3:
        st.markdown("**Pense-bete Lundi matin**")
        if HAS_CHEAT_SHEET and not report.empty:
            try:
                cheat_data = _generate_cheat_sheet(report, market_status)
                if cheat_data:
                    fn = "cheatsheet_" + datetime.now().strftime("%Y%m%d") + ".xlsx"
                    st.download_button(
                        "Cheat Sheet Lundi",
                        data=cheat_data,
                        file_name=fn,
                ...
                st.caption("Cheat sheet indisponible")
