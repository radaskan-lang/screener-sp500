...
if st.button(f" Lancer  S&P 500 complet ({len(SP500_TICKERS)} actions)"):
    ...
    if report.empty:
        ...
        for idx, row in report.iterrows():
            ...
            st.markdown(
                f'<div style="border:2px solid {border};border-radius:10px;padding:14px;margin-bottom:10px;background:#111827;">'
                f'<b style="color:{rank_color}">#{rank}</b>'
                f' <b style="color:#4a90d0;">{row.get("Ticker","")}</b>'
                f' <span style="color:#94a3b8;">{row.get("Sector","")}</span>'
...
if "scan_report" in st.session_state and not st.session_state["scan_report"].empty:
    _rpt=st.session_state["scan_report"]; _reg=st.session_state.get("scan_regime","HAUSSIER")
    _dff=st.session_state.get("scan_df",_rpt)
    st.markdown("---"); st.markdown("## Top Trades")
    if HAS_SIMULATIONS:
        _so={v["name"]:k for k,v in SIMULATION_CONFIGS.items()}
        _sn=st.selectbox("Simulation",list(_so.keys()),key="sim_sel")
        _sk=_so[_sn]; _sc=SIMULATION_CONFIGS[_sk]
        st.caption(f"Score:{_sc['score_min']}+ Conv:{_sc['conv_min']}/7 R/R:{_sc['rr_min']} ADX:{_sc['adx_min']} Fib:{'ON' if _sc['fibonacci'] else 'OFF'}")
        _rs=filter_for_simulation(_dff,_sk)
    else:
        _sk="Sim1"; _rs=_rpt
    st.markdown(f"**Marche {_reg} | {_sk}** | Selectionner :")
    _sel=[]
    for _i,_r in _rs.iterrows():
        _rk=_i+1; _tk=str(_r.get("Ticker",""))
        _sc2=_r.get("Score_Final",0); _en=_r.get("Entree",0)
        ...
        _cb.markdown(f"**#{_rk} {_tk}** | Score:`{_sc2}` Conv:`{_cv}/7` ADX:`{_ax}` OBV:`{_ob}`  \\nEntree:`${_en}` Stop:`${_st2}` Target:`${_tg}` R/R:`{_rr}:1`")
        if _ck: _sel.append({"ticker":_tk,"entry_price":_en,"stop_price":_st2,"target_price":_tg,"conv_n":_cv,"score":_sc2,"sector":_se,"simulation":_sk})
    if _sel:
