...
def fetch(ticker):
    try:
        ...
        return {
            ...
            "FIB_Level":       fib_data["current_level"],
            "FIB_Support":     fib_data["nearest_support"][1] if fib_data["nearest_support"] else None,
            "FIB_Resist":      fib_data["nearest_resist"][1] if fib_data["nearest_resist"] else None,
    ...
    try:
        ...
        if fib_score != 0:
            ...
            if fib_signal and fib_signal not in ["None","—",""]:
                reasons.append(f"📐 {fib_signal[:45]}")
    except Exception:
        pass
...
if st.button(f"🔄 Lancer — S&P 500 complet ({len(SP500_TICKERS)} actions)"):
    ...
    if report.empty:
        ...
        for idx, row in report.iterrows():
            ...
            fib_level   = str(row.get("FIB_Level", "") or "")
            fib_support = row.get("FIB_Support", None)
            fib_resist  = row.get("FIB_Resist", None)
            if fib_badge and fib_badge not in ["—", ""]:
                fib_color = "#00ff88" if (row.get("FIB_Score",0) or 0)>0 else "#f87171"
                st.markdown(
                    f"**📐 Fibonacci :** <span style='color:{fib_color};'>{fib_badge}</span> — Niveau: `{fib_level}`",
                    unsafe_allow_html=True
                )
                fib_parts = []
                if fib_support: fib_parts.append(f"Support Fib: `${fib_support}`")
                if fib_resist:  fib_parts.append(f"Résistance Fib: `${fib_resist}`")
                if fib_parts: st.markdown(" &nbsp;|&nbsp; ".join(fib_parts))

            # Bollinger avancé
            bb_badge  = str(row.get("BB_Badge", "") or "")
            bb_trend  = str(row.get("BB_WidthTrend", "") or "")
            if bb_badge and bb_badge not in ["—", ""]:
                bb_color   = "#00ff88" if (row.get("BB_Score",0) or 0)>0 else "#f87171"
                trend_icon = "📈" if bb_trend=="EXPANDING" else "📉" if bb_trend=="CONTRACTING" else "~"
                st.markdown(
                    f"**📊 Bollinger :** <span style='color:{bb_color};'>{bb_badge}</span> {trend_icon} `{bb_trend}`",
                    unsafe_allow_html=True
                )
    ...
    cols_display = [
        ...
        "FIB_Badge","FIB_Signal","FIB_Level","FIB_Support","FIB_Resist",
