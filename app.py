...
try:
    ...
    GITHUB_STORAGE=github_storage_available()
except Exception:
...
try:
    from simulations import SIMULATION_CONFIGS,filter_for_simulation,get_simulation_summary
    HAS_SIMULATIONS=True
except Exception:
    HAS_SIMULATIONS=False; SIMULATION_CONFIGS={}
...
SP500_TICKERS = [
    ...
    "ALGN","LNT","ALL","GOOGL","GOOG","MO","AMZN","AMCR","AEE","AEP","AXP","AIG","AMT",
    "AWK","AMP","AME","AMGN","APH","ADI","AON","APA","APO","AAPL","AMAT","APTV","ACGL",
    "ADM","ANET","AJG","AIZ","T","ATO","ADSK","ADP","AZO","AVB","AVY","AXON","BKR","BALL","BAC",
    "BAX","BDX","BRK-B","BBY","BIIB","BLK","BX","BA","BSX","BMY","AVGO","BR","BRO","BF-B",
    "BLDR","BXP","CHRW","CDNS","CZR","CPT","CPB","COF","CAH","KMX","CCL","CARR","CAT","CBOE",
    "CBRE","CDW","CE","COR","CNC","CF","CRL","SCHW","CHTR","CVX","CMG","CB","CHD","CI",
    ...
    "CVS","DHR","DRI","DVA","DECK","DE","DELL","DAL","DVN","DXCM","FANG","DLR","DG",
    ...
    "HCA","DOC","HSIC","HSY","HPE","HLT","HD","HON","HRL","HST","HWM","HPQ","HUBB",
    ...
    "LLY","LIN","LYV","LKQ","LMT","L","LOW","LULU","LYB","MTB","MPC","MKTX","MAR","MMC",
    ...
    "PKG","PANW","PH","PAYX","PAYC","PYPL","PNR","PEP","PFE","PCG","PM","PSX","PNC","POOL",
    ...
    "VRSN","VRSK","VZ","VRTX","VTRS","VICI","V","VST","VMC","WRB","GWW","WAB","WMT","DIS",
    "WBD","WM","WAT","WEC","WFC","WELL","WST","WDC","WY","WMB","WTW","WYNN","XEL","XYL","YUM",
    "ZBRA","ZBH","ZTS"
]

NASDAQ100_EXTRA=[
    "ASML","MELI","PANW","CRWD","FTNT","DDOG","SNOW","ZS","OKTA",
    "WDAY","TEAM","COIN","RBLX","UBER","PLTR","SOFI","AFRM","RIVN",
    "SMCI","GEHC","SGEN","ILMN","ENPH","ABNB","MTCH","PINS","NXPI",
    "MCHP","LULU","BKNG","SIRI","CHTR","FANG","ON","LCID","SPLK",
]

RUSSELL200_EXTRA = [
    "CHWY","JOBY","ARRY","PCVX","RARE","AGIO","KRTX","RXRX","BEAM","NTLA",
    "EDIT","CRSP","BLUE","ACAD","INVA","PRTA","IMVT","ARWR","IONS","FOLD",
    "ALNY","NBIX","HALO","EXEL","VKTX","INSM","PTGX","RCUS","TMDX","NARI",
    "AAON","MGNI","RGEN","IOVA","DNLI","ADMA","EVTC","GTLS","PRIM","SPSC",
    "ACLX","GKOS","ITCI","KRYS","LGND","NVAX","OCUL","SAGE","TARS","XNCR",
]
...
def calc_adx(hist,period=14):
    try:
        if hist is None or len(hist)<period+5: return 0.0,0.0,0.0
        high=hist["High"]; low=hist["Low"]; close=hist["Close"]
        tr=pd.concat([high-low,(high-close.shift(1)).abs(),(low-close.shift(1)).abs()],axis=1).max(axis=1)
        dmp=high.diff().clip(lower=0); dmm=(-low.diff()).clip(lower=0)
        dmp=dmp.where(dmp>dmm,0); dmm=dmm.where(dmm>dmp,0)
        atr=tr.ewm(span=period,adjust=False).mean()
        dip=100*dmp.ewm(span=period,adjust=False).mean()/atr.clip(lower=1e-10)
        dim=100*dmm.ewm(span=period,adjust=False).mean()/atr.clip(lower=1e-10)
        dx=100*(dip-dim).abs()/(dip+dim).clip(lower=1e-10)
        adx=dx.ewm(span=period,adjust=False).mean()
        return round(float(adx.iloc[-1]),1),round(float(dip.iloc[-1]),1),round(float(dim.iloc[-1]),1)
    except Exception: return 0.0,0.0,0.0

def calc_obv(hist):
    try:
        if hist is None or len(hist)<10: return 0.0,"NEUTRE"
        close=hist["Close"]; volume=hist["Volume"]
        obv=(volume*close.diff().apply(lambda x:1 if x>0 else -1 if x<0 else 0)).cumsum()
        obv_ma=obv.rolling(20).mean()
        obv_5d=float(obv.iloc[-1])-float(obv.iloc[-6]) if len(obv)>=6 else 0
        if obv.iloc[-1]>obv_ma.iloc[-1] and obv_5d>0: trend="HAUSSIER"
        elif obv.iloc[-1]<obv_ma.iloc[-1] and obv_5d<0: trend="BAISSIER"
        else: trend="NEUTRE"
        return round(float(obv.iloc[-1])/1e6,2),trend
    except Exception: return 0.0,"NEUTRE"
...
def detect_volume_anomaly(hist):
    try:
        ...
        score=0; signal=None
        if vol_ratio>=3.0 and price>prev: score=20; signal=f"VOLUME CLIMAX ({vol_ratio}x)"
        elif vol_ratio>=2.0 and price>prev: score=15; signal=f"Volume spike ({vol_ratio}x)"
        elif vol_ratio>=1.5:
            score=10; signal=f"Volume fort ({vol_ratio}x)"
            if len(volume)>=5:
                vols=volume.iloc[-5:].values
                if all(vols[i]<=vols[i+1] for i in range(4)): score=18; signal="QUIET BUILDUP"
...
def fetch(ticker):
    try:
        ...
        ma50=float(close.rolling(50).mean().iloc[-1]); ma200=float(close.rolling(200).mean().iloc[-1])
        ma20=float(close.rolling(20).mean().iloc[-1])
        ma20_cross=len(close)>=22 and float(close.iloc[-2])<float(close.rolling(20).mean().iloc[-2]) and float(close.iloc[-1])>ma20
        adx_val,di_plus,di_minus=calc_adx(hist)
        obv_val,obv_trend=calc_obv(hist)
        rsi       = calc_rsi(close)
        ...
        try:
            info=t.info; revenue_gr=info.get("revenueGrowth",None); sector=info.get("sector","N/A")
        except Exception:
            revenue_gr=None; sector="N/A"

        return {
            "Ticker":        ticker,
            "Sector":        sector,
            "Prix":          round(price, 2),
            "MA50":round(ma50,2),"MA200":round(ma200,2),"MA20":round(ma20,2),"MA20_Cross":ma20_cross,"ADX":adx_val,"DI_Plus":di_plus,"DI_Minus":di_minus,"OBV_Trend":obv_trend,
            "RSI":           round(rsi, 1),
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("---")
    st.markdown("**Univers de titres :**")
    include_nasdaq = st.checkbox("Nasdaq 100", value=st.session_state.get("inc_ndx_val",False), key="inc_ndx", help="Ajoute ~35 titres")
    st.session_state["inc_ndx_val"] = include_nasdaq
    include_russell = st.checkbox("Russell 200", value=st.session_state.get("inc_russ_val",False), key="inc_russ", help="Ajoute ~50 titres mid-cap")
    st.session_state["inc_russ_val"] = include_russell
...
if st.button(f" Lancer scan ({len(SP500_TICKERS)+(len(NASDAQ100_EXTRA) if st.session_state.get('inc_ndx_val') else 0)+(len(RUSSELL200_EXTRA) if st.session_state.get('inc_russ_val') else 0)} actions)"):

    tickers_to_analyze=SP500_TICKERS+(NASDAQ100_EXTRA if st.session_state.get("inc_ndx_val",False) else [])+(RUSSELL200_EXTRA if st.session_state.get("inc_russ_val",False) else [])
    ...
    if use_prefilter:
        ...
        pf_stat = st.empty()
        def pf_cb(pct, msg=""):
            try: pf_prog.progress(min(1.0,max(0.0,float(pct))))
            except Exception: pass
            if msg: pf_stat.caption(msg)
        ...
        st.markdown(f"""<div class="prefilter-banner">
            ...
            &nbsp;/&nbsp; {pf_result['n_total']}
            &nbsp;|&nbsp; {pf_result['n_failed']} limines
            &nbsp;|&nbsp; {pf_result['pass_rate']}% passage
        </div>""", unsafe_allow_html=True)
        with st.expander(f" {pf_result['n_failed']} actions limines"):
            st.dataframe(pd.DataFrame([
                {"Ticker": t, "Raison": pf_result['reasons'][t]}
                for t in pf_result['failed']
    ...
    conv_res=df.apply(lambda r:calc_convergence(r),axis=1)
    df["Conv_N"]=conv_res.apply(lambda x:x.get("n_signals",0))
    df["Conv_Bar"]=conv_res.apply(lambda x:x.get("bar",""))
    df["Conv_Label"]=conv_res.apply(lambda x:x.get("conv_label",""))
    df["Conv_Color"]=conv_res.apply(lambda x:x.get("conv_color","#64748b"))
    df["Conv_On"]=conv_res.apply(lambda x:" | ".join(x.get("signals_on",[])))
    df["Conv_Off"]=conv_res.apply(lambda x:" | ".join(x.get("signals_off",[])))
    ...
    df["Score_Final"]=df["AI Score Ajuste"]
    ...
    report=build_trade_report(df,top_n=top_n,min_signals=min_signals,min_rr=min_rr_conv)
    ...
    if report.empty:
        ...
        for idx, row in report.iterrows():
            ...
            # Fiche native
            border = "#ffd700" if n_sig>=6 else "#00ff88" if n_sig>=5 else "#1e3a5f"
            st.markdown(f'<div style="border:2px solid {border};border-radius:10px;padding:14px;margin-bottom:10px;background:#111827;"><b style="color:{rank_color}">#{rank}</b> <b style="color:#4a90d0;">{row.get("Ticker","")}</b> <span style="color:#94a3b8;">{row.get("Sector","")}</span><span style="float:right;color:#00ff88;">{conv_lbl}</span></div>', unsafe_allow_html=True)
            c1,c2,c3,c4=st.columns(4)
            c1.metric("Prix actuel",f"${row.get('Prix',0)}")
            c2.metric("Score",f"{score_fin}/100")
            c3.metric("Conv",f"{n_sig}/7")
            c4.metric("R/R",f"{rr}:1" if rr else "--")
            # Niveaux de trading
            entry = row.get('Entree', row.get('Prix', 0))
            stop  = row.get('Stop', 0)
            target= row.get('Target', 0)
            try:
                stop_pct   = round((float(stop)-float(entry))/float(entry)*100,2) if float(entry)>0 else 0
                target_pct = round((float(target)-float(entry))/float(entry)*100,2) if float(entry)>0 else 0
            except Exception:
                stop_pct = target_pct = 0
            t1,t2,t3=st.columns(3)
            t1.metric("ENTREE lundi", f"${entry}", help="Prix d'entree suggere lundi matin")
            t2.metric("STOP-LOSS", f"${stop}", f"{stop_pct}%", delta_color="inverse")
            t3.metric("VENTE cible", f"${target}", f"+{target_pct}%")
            if signals_on: st.caption("Signaux: "+signals_on[:150])
    ...
    st.dataframe(df_filtered[cols_display], width="stretch", height=400)
...
# Cases a cocher
if "scan_report" in st.session_state and not st.session_state["scan_report"].empty:
    _rpt=st.session_state["scan_report"]
    _reg=st.session_state.get("scan_regime","HAUSSIER")
    _dff=st.session_state.get("scan_df",_rpt)
    st.markdown("---")
    st.markdown("## Top Trades")
    st.caption(f"Marche {_reg} -- {len(_rpt)} titres identifies")
    if HAS_SIMULATIONS:
        _so={v["name"]:k for k,v in SIMULATION_CONFIGS.items()}
        _opts=["Tous les titres (sans filtre)"]+list(_so.keys())
        _sn=st.selectbox("Simulation / Filtre",_opts,key="sim_sel")
        if _sn=="Tous les titres (sans filtre)":
            _sk="Sim1"; _rs=_rpt.head(10)
        else:
            _sk=_so[_sn]; _sc2=SIMULATION_CONFIGS[_sk]
            st.caption(f"Score:{_sc2['score_min']}+ Conv:{_sc2['conv_min']}/7 ADX:{_sc2['adx_min']} Fib:{'ON' if _sc2['fibonacci'] else 'OFF'}")
            _rs=filter_for_simulation(_dff,_sk)
            if _rs.empty:
                st.warning(f"Aucun titre ne passe {_sk} -- affichage top 10")
                _rs=_rpt.head(10)
    else:
        _sk="Sim1"; _rs=_rpt.head(10)
    _sel=[]
    for _i,_r in _rs.iterrows():
        _rk=_i+1; _tk=str(_r.get("Ticker",""))
        _sc3=_r.get("Score_Final",0); _en=_r.get("Entree",0)
        _st2=_r.get("Stop",0); _tg=_r.get("Target",0); _rr=_r.get("RR_Ratio",0)
        _se=str(_r.get("Sector","N/A")); _cv=int(_r.get("Conv_N",0))
        _ax=_r.get("ADX",0); _ob=str(_r.get("OBV_Trend","") or "")
        _ca,_cb=st.columns([0.12,0.88])
        _ck=_ca.checkbox("select",key=f"ptc_{_sk}_{_tk}_{_rk}",label_visibility="hidden")
        _cb.markdown(f"**#{_rk} {_tk}** | Score:`{_sc3}` Conv:`{_cv}/7` ADX:`{_ax}` OBV:`{_ob}`  \\nEntree:`${_en}` Stop:`${_st2}` Target:`${_tg}` R/R:`{_rr}:1`")
        if _ck: _sel.append({"ticker":_tk,"entry_price":_en,"stop_price":_st2,"target_price":_tg,"conv_n":_cv,"score":_sc3,"sector":_se,"simulation":_sk})
    if _sel:
        _str=st.selectbox("Strategie",["A +5%","B +7%","C Vendredi","D Stop 3%","E Stop 5%","T3 Trailing 3%","T5 Trailing 5%"],index=2,key=f"pt_s_{_sk}")
        if st.button(f"Ajouter {len(_sel)} trade(s) - {_sk}",key=f"pt_a_{_sk}"):
            for _t in _sel:
                add_paper_trade(ticker=_t["ticker"],entry_price=_t["entry_price"],stop_price=_t["stop_price"],target_price=_t["target_price"],conv_n=_t["conv_n"],score=_t["score"],strategy=_str,sector=_t["sector"],week_date=datetime.now().strftime("%Y-W%V"))
            st.success(f"{len(_sel)} trade(s) ajoutes - {_sk}!"); st.rerun()

# PAPER TRADING + JOURNAL
st.markdown("---")
st.markdown("## Paper Trading & Journal")

pt_tab, auto_tab, journal_tab = st.tabs(["Paper Trading", "Generation Auto", "Journal Reel"])
...
with pt_tab:
    ...
    if open_trades:
        ...
        st.dataframe(rows_open, width="stretch")
        ...
        with st.expander(f"Historique ({len(closed_trades)} trades fermes)"):
            ...
            for t in closed_trades:
                ...
                rows_cl.append({"Ticker":t.get("ticker"),"Entree":f"${ep}",
                    ...
                    "Resultat":"WIN" if pnl>=0 else "LOSS",
                    "Strat":t.get("strategy"),"Semaine":t.get("week")})
            st.dataframe(rows_cl, width="stretch")

with auto_tab:
    st.markdown("### Generation automatique de trades")
    if "scan_df" not in st.session_state or st.session_state["scan_df"].empty:
        st.warning("Lance d'abord un scan pour generer les trades automatiquement.")
    else:
        _df_auto = st.session_state["scan_df"]
        st.caption(f"{len(_df_auto)} titres disponibles depuis le dernier scan")
        st.markdown("**Simulations a inclure :**")
        _ac1,_ac2,_ac3,_ac4 = st.columns(4)
        _use_s1 = _ac1.checkbox("Sim1 Baseline", value=True, key="auto_s1")
        _use_s2 = _ac2.checkbox("Sim2 Momentum", value=True, key="auto_s2")
        _use_s3 = _ac3.checkbox("Sim3 Large",    value=True, key="auto_s3")
        _use_s4 = _ac4.checkbox("Sim4 Strict",   value=False, key="auto_s4")
        st.markdown("**Strategies a inclure :**")
        _bc1,_bc2,_bc3,_bc4,_bc5,_bc6,_bc7 = st.columns(7)
        _use_a  = _bc1.checkbox("A +5%",      value=False, key="auto_a")
        _use_b  = _bc2.checkbox("B +7%",      value=False, key="auto_b")
        _use_c  = _bc3.checkbox("C Vendredi", value=True,  key="auto_c")
        _use_d  = _bc4.checkbox("D Stop 3%",  value=False, key="auto_d")
        _use_e  = _bc5.checkbox("E Stop 5%",  value=False, key="auto_e")
        _use_t3 = _bc6.checkbox("T3 Trail",   value=True,  key="auto_t3")
        _use_t5 = _bc7.checkbox("T5 Trail",   value=False, key="auto_t5")
        # Calculate total
        _sims_sel = []
        if _use_s1 and HAS_SIMULATIONS: _sims_sel.append("Sim1")
        if _use_s2 and HAS_SIMULATIONS: _sims_sel.append("Sim2")
        if _use_s3 and HAS_SIMULATIONS: _sims_sel.append("Sim3")
        if _use_s4 and HAS_SIMULATIONS: _sims_sel.append("Sim4")
        if not _sims_sel: _sims_sel = ["Sim1"]
        _strats_sel = []
        if _use_a:  _strats_sel.append("A +5%")
        if _use_b:  _strats_sel.append("B +7%")
        if _use_c:  _strats_sel.append("C Vendredi")
        if _use_d:  _strats_sel.append("D Stop 3%")
        if _use_e:  _strats_sel.append("E Stop 5%")
        if _use_t3: _strats_sel.append("T3 Trailing 3%")
        if _use_t5: _strats_sel.append("T5 Trailing 5%")
        if not _strats_sel: _strats_sel = ["C Vendredi"]
        # Preview count
        _total_preview = 0
        _preview_lines = []
        for _sk in _sims_sel:
            if HAS_SIMULATIONS:
                _rs = filter_for_simulation(_df_auto, _sk)
            else:
                _rs = _df_auto.head(10)
            _n = len(_rs)
            _total_preview += _n * len(_strats_sel)
            _preview_lines.append(f"{_sk}: {_n} titres x {len(_strats_sel)} strategies = {_n*len(_strats_sel)} trades")
        st.markdown("**Preview :**")
        for line in _preview_lines:
            st.caption(line)
        st.markdown(f"**Total : {_total_preview} trades a creer**")
        if st.button(f"Generer {_total_preview} trades", key="auto_gen", type="primary"):
            _count = 0
            _errors = 0
            _prog = st.progress(0)
            _week = datetime.now().strftime("%Y-W%V")
            # Charger UNE SEULE FOIS depuis GitHub
            from trading_tools import load_paper_trades, save_paper_trades
            _all_trades = load_paper_trades()
            for _sk in _sims_sel:
                if HAS_SIMULATIONS:
                    _rs = filter_for_simulation(_df_auto, _sk)
                else:
                ...
                for _, _r in _rs.iterrows():
                    for _str in _strats_sel:
                        try:
                            _trade = {
                                "id": f"{_r.get('Ticker','')}_{_week}_{_sk}_{_str[:2]}_{_count}",
                                "ticker": str(_r.get("Ticker","")),
                                "week": _week,
                                "simulation": _sk,
                                "entry_price": round(float(_r.get("Entree", _r.get("Prix",0))),2),
                                "stop_price": round(float(_r.get("Stop",0)),2),
                                "target_price": round(float(_r.get("Target",0)),2),
                                "conv_n": int(_r.get("Conv_N",0)),
                                "score": float(_r.get("Score_Final",0)),
                                "strategy": _str,
                                "sector": str(_r.get("Sector","N/A")),
                                "status": "OPEN",
                                "exit_price": None,
                                "exit_date": None,
                                "pnl_pct": None,
                            ...
                            _all_trades.append(_trade)
                            _count += 1
                            _prog.progress(min(1.0, _count/max(_total_preview,1)))
                        except Exception:
                            _errors += 1
            # Sauvegarder UNE SEULE FOIS
            save_paper_trades(_all_trades)
            st.success(f"{_count} trades crees et sauvegardes!")
            if _errors: st.warning(f"{_errors} erreurs ignorees")
            st.rerun()
    ...
    if open_j:
        ...
        st.dataframe(rows_j, width="stretch")
        ...
        with st.expander(f"Historique reel ({len(closed_j)} trades)"):
            ...
            for t in closed_j:
                ...
                rows_jc.append({"Ticker":t.get("ticker"),"Date":t.get("date_entry"),
                    ...
                    "Resultat":"WIN" if pnl>=0 else "LOSS",
                    "Notes":t.get("notes","")})
            st.dataframe(rows_jc, width="stretch")
