...
def ai_score(row):
    ...
    try:
        ...
        if er == "HIGH":
            score -= 25; reasons.append("🔴 Earnings cette semaine — risque élevé")
        elif er == "MEDIUM":
            score -= 15; reasons.append("⚠️ Earnings proches — prudence")
    except Exception:
        pass
    ...
    try:
        ...
        if sr_pos >= 95:
            score -= 10; reasons.append("⚠️ Proche du High 52w — résistance")
        elif sr_pos >= 80:
    ...
    try:
        ...
        if fib_valid == False:
            score -= 10
            reasons.append(f"🔴 FIB: Résistance dans {fib_dist_r}%")
    except Exception:
        pass

    # Bollinger avancé (max +15, min -10)
    try:
    ...
    try:
        ...
        if id_conf == "CONTRADICTOIRE":
            score -= 10; reasons.append("⚠️ TF contradictoires")
        elif id_conf == "FORT" and id_total > 10:
            reasons.append("✅ Multi-TF alignés + Momentum")
    except Exception:
        pass

    # Marché BAISSIER — pénalité globale
    try:
        regime = st.session_state.get("regime","HAUSSIER")
        if regime == "BAISSIER":
            score -= 20; reasons.append("🔴 Marché BAISSIER — prudence")
    except Exception:
        pass
...
if st.button(f"\ud83d\udd04 Lancer \u2014 S&P 500 complet ({len(SP500_TICKERS)} actions)"):
    ...
    if report.empty:
        ...
        for idx, row in report.iterrows():
            ...
            if fib_dist_r and fib_dist_r < 99: fib_parts.append(f"Dist résistance: `{fib_dist_r}%`")
            if fib_parts: st.markdown(" &nbsp;|&nbsp; ".join(fib_parts))
            if fib_stop_v and fib_target_v:
                rr_str = f" &nbsp;|&nbsp; R/R Fib: `{fib_rr_v}:1`" if fib_rr_v else ""
                st.markdown(f"🛡️ Stop Fib: `${fib_stop_v}` &nbsp;|&nbsp; 🎯 Target Fib: `${fib_target_v}`{rr_str}")
