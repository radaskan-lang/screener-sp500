...
if "scan_report" in st.session_state and not st.session_state["scan_report"].empty:
    ...
    if _selected:
        ...
        if st.button(f"Ajouter {len(_selected)} trade(s) - {_sim_key}",key=f"pt_add_{_sim_key}"):
            for _t in _selected:
                add_paper_trade(ticker=_t["ticker"],entry_price=_t["entry_price"],stop_price=_t["stop_price"],target_price=_t["target_price"],conv_n=_t["conv_n"],score=_t["score"],strategy=_strat,sector=_t["sector"],week_date=datetime.now().strftime("%Y-W%V"),simulation=_t.get("simulation","Sim1"))
