...
# Les 7 signaux verifies pour la convergence
SIGNAL_CHECKS = [
    "trend",    # MA50 / MA200 alignees
    "ma20",     # MA20 momentum court terme
    "rsi",      # RSI zone ideale swing
    "macd",     # MACD histogramme haussier
    "volume",   # Volume ratio fort
    "pattern",  # Pattern technique detecte
...
def calc_convergence(row):
    ...
    try:
        ...
        # ── 2. MA20 — momentum court terme ──
        try:
            ma20       = float(row.get("MA20", 0) or 0)
            ma20_cross = bool(row.get("MA20_Cross", False))
            if ma20 > 0:
                if ma20_cross:
                    signals_on.append("Recassement MA20 a la hausse")
                    details["ma20"] = {"ok": True, "weight": 2, "detail": f"Vient de recasser MA20 ${round(ma20,2)}"}
                elif price > ma20 > ma50:
                    signals_on.append(f"Prix > MA20 > MA50 (alignement fort)")
                    details["ma20"] = {"ok": True, "weight": 2, "detail": f"${round(price,2)} > MA20 ${round(ma20,2)} > MA50 ${round(ma50,2)}"}
                elif price > ma20:
                    signals_on.append(f"Prix au-dessus MA20 (${round(ma20,2)})")
                    details["ma20"] = {"ok": True, "weight": 1, "detail": f"Prix ${round(price,2)} > MA20 ${round(ma20,2)}"}
                else:
                    signals_off.append(f"Sous MA20 (${round(ma20,2)})")
                    details["ma20"] = {"ok": False, "weight": 0, "detail": f"Prix ${round(price,2)} < MA20 ${round(ma20,2)}"}
            else:
                details["ma20"] = {"ok": False, "weight": 0, "detail": "MA20 non disponible"}
        except Exception:
            details["ma20"] = {"ok": False, "weight": 0, "detail": "Erreur MA20"}
