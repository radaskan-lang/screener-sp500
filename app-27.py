...
def fetch(ticker):
    try:
        ...
        print(f"FETCH ERROR {ticker}: {str(e)[:80]}")
        return None
...
if st.button(f" Lancer  S&P 500 complet ({len(SP500_TICKERS)} actions)"):
    ...
    # Calcul convergence pour chaque titre
    conv_results = df.apply(lambda row: calc_convergence(row), axis=1)
    df["Conv_N"]     = conv_results.apply(lambda x: x.get("n_signals", 0))
    df["Conv_Bar"]   = conv_results.apply(lambda x: x.get("bar", ""))
    df["Conv_Label"] = conv_results.apply(lambda x: x.get("conv_label", ""))
    df["Conv_Color"] = conv_results.apply(lambda x: x.get("conv_color", "#64748b"))
    df["Conv_On"]    = conv_results.apply(lambda x: " | ".join(x.get("signals_on", [])))
    df["Conv_Off"]   = conv_results.apply(lambda x: " | ".join(x.get("signals_off", [])))
    ...
    if report.empty:
        ...
        for idx, row in report.iterrows():
            ...
            # Fiche native Streamlit (pas de HTML)
            border_color = "#ffd700" if n_sig == 6 else "#00ff88" if n_sig >= 5 else "#1e3a5f"
            st.markdown(f"""
            <div style="border:2px solid {border_color};border-radius:12px;padding:16px;margin-bottom:12px;background:#111827;">
            <b style="color:{rank_color};font-size:1.2rem;">#{rank}</b>
            <b style="color:#4a90d0;font-size:1.1rem;margin-left:8px;">{row.get("Ticker","")}</b>
            <span style="color:#94a3b8;font-size:0.85rem;margin-left:8px;">{row.get("Sector","")}</span>
            <span style="float:right;color:#00ff88;font-size:0.8rem;">{conv_lbl}</span>
            </div>""", unsafe_allow_html=True)
            # Metriques principales
            m1,m2,m3,m4 = st.columns(4)
            m1.metric("Prix", f"${row.get('Prix',0)}")
            m2.metric("Score", f"{score_fin}/100")
            m3.metric("Conv", f"{n_sig}/6")
            m4.metric("R/R", f"{rr}:1" if rr else "--")
            # Niveaux
            ...
            if signals_on:
                ...
                for sig in signals_on.split(" | ")[:4]:
                    if sig.strip(): st.markdown(f"- {sig.strip()}")
...
# Show rapport from session_state (persists between interactions)
if "scan_df" in st.session_state and not st.session_state["scan_df"].empty:
    _df     = st.session_state["scan_df"]
    _regime = st.session_state.get("scan_regime", "HAUSSIER")
    _top_n  = 10
    # Utiliser le rapport filtre si disponible, sinon top_n du df
    if "scan_report" in st.session_state and not st.session_state["scan_report"].empty:
        _report = st.session_state["scan_report"]
    else:
        _report = _df.head(_top_n)

    st.markdown("---")
    st.markdown(f"## Rapport Top {_top_n} Trades")
    st.markdown(f"**Semaine du {datetime.now().strftime(chr(37)+chr(100)+chr(32)+chr(37)+chr(66)+chr(32)+chr(37)+chr(89))}  Marche {_regime}**")

    # Cases a cocher pour paper trading
    st.markdown("**Selectionner pour Paper Trading :**")
    _selected = []

    for _idx, _row in _report.iterrows():
        _rank   = _idx + 1
        _ticker = str(_row.get("Ticker",""))
        _score  = _row.get("Score_Final", _row.get("AI Score Ajuste", 0))
        _entry  = _row.get("Entree", _row.get("Prix", 0))
        _stop   = _row.get("Stop", 0)
        _target = _row.get("Target", 0)
        _rr     = _row.get("RR_Ratio", 0)
        _sector = str(_row.get("Sector","N/A"))
        _conv   = int(_row.get("Conv_N", 0))

        _cc, _ci = st.columns([0.12, 0.88])
        _chk = _cc.checkbox("", key=f"ptc_{_ticker}_{_rank}", label_visibility="collapsed")
        _ci.markdown(
            f"**#{_rank} {_ticker}** | Score: `{_score}` | Conv: `{_conv}/6`  \n"
            f"Entree: `${_entry}` Stop: `${_stop}` Target: `${_target}` R/R: `{_rr}:1`"
        )
        if _chk:
            _selected.append({"ticker":_ticker,"entry_price":_entry,"stop_price":_stop,
                "target_price":_target,"conv_n":_conv,"score":_score,"sector":_sector})

    if _selected:
        _strat = st.selectbox("Strategie", ["A +5%","B +7%","C Vendredi","D Stop 3%","E Stop 5%"], index=2, key="pt_ss_strat")
        if st.button(f"Ajouter {len(_selected)} trade(s) au Paper Trading", key="pt_ss_add"):
            for _t in _selected:
                add_paper_trade(ticker=_t["ticker"], entry_price=_t["entry_price"],
                    stop_price=_t["stop_price"], target_price=_t["target_price"],
                    conv_n=_t["conv_n"], score=_t["score"], strategy=_strat,
                    sector=_t["sector"], week_date=datetime.now().strftime("%Y-W%V"))
            st.success(f"{len(_selected)} trade(s) ajoutes!")
            st.rerun()
